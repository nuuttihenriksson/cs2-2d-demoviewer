...
class RoundNav extends Component {
  constructor(props) {
    ...
    this.messageBus.listen([MSG_INIT_ROUNDS], function (msg) {
      let rounds = []
      msg.rounds.forEach(r => console.log(r.winner === ""))
  ...
  render() {
    return (
        <div className="w3-bar w3-small w3-center-align">
          {this.state.rounds}
        </div>
    );
  }
...
class Round extends Component {
  ...
  render() {
    return <button className={`w3-button roundNav ${this.props.winner === "" ? "NoWin" : this.props.winner} ${this.state.active ? "active" : ""}`}
